...
Happy Post runs in one sandbox VPC spanning two Availability Zones. Each zone contains a public subnet, private application subnet, and private database subnet. The internet-facing HTTPS ALB spans the public subnets. ECS Fargate tasks run only in private application subnets, and Aurora PostgreSQL Serverless uses a private DB subnet group across the database subnets.

The Internet Gateway serves the public ALB and the one sandbox NAT Gateway. Private application tasks use the NAT Gateway for approved outbound ECR, CloudWatch, and external-dependency access. Aurora has no public IP and no public route.

Security groups enforce this path: Internet to ALB on HTTPS 443; ALB to frontend and backend task security groups on their application ports; backend task security group to Aurora on TCP 5432. The frontend and ALB have no direct path to Aurora.
