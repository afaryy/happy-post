...
| Image scanning and registry | The applied platform root defines one private immutable, scan-on-push ECR repository per workload; automatic and main-only manual publication block HIGH/CRITICAL image findings and record verified SHA-256 digest/source-commit artifacts for the pending service roots | [Architecture](architecture.md), [Security decisions](security-decisions.md) |
| ECS delivery | One applied cluster will host two independently bootstrapped Fargate services behind the applied ALB; manual bootstrap verifies a component digest and immutable source-commit tag in its matching ECR repository before applying its root | [Architecture](architecture.md), [Deployment and rollback](deployment-and-rollback.md) |
...
This table records the approved design, completed application MVPs, local P3 containerisation, applied Terraform network, RDS data, platform, and edge foundations, and CI controls. The service roots are defined but await their documented digest-safe bootstrap sequence. Application database persistence remains pending an approved workload implementation.
