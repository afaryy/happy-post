...
| Image scanning and registry | The applied platform root defines one private immutable, scan-on-push ECR repository per workload; the pending service roots require scanned SHA-256 digests | [Architecture](architecture.md), [Security decisions](security-decisions.md) |
| ECS delivery | One applied cluster will host two independently applied Fargate services behind the pending ALB | [Architecture](architecture.md), [Deployment and rollback](deployment-and-rollback.md) |
| Public domain and TLS | Cloudflare delegates the application subdomain to Route 53; the pending edge root validates ACM DNS and terminates HTTPS at ALB | [Architecture](architecture.md) |
...
This table records the approved design, completed application MVPs, local P3 containerisation, applied Terraform network, RDS data, and platform foundations, and CI controls. The edge and service roots are defined but await their documented digest-safe apply sequence. Application database persistence remains pending an approved workload implementation.
