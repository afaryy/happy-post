...
variable "frontend_image_digest" {
  description = "Required SHA-256 digest of the scanned frontend ECR image."
  type        = string
  nullable    = false

  validation {
    condition     = can(regex("^sha256:[0-9a-f]{64}$", var.frontend_image_digest))
    error_message = "frontend_image_digest must be a lowercase SHA-256 image digest, never a tag."
  }
}
